...
namespace
{
    vk::DebugUtilsMessengerCreateInfoEXT GetDebugMessengerCreateInfo(PFN_vkDebugUtilsMessengerCallbackEXT pDebugCallback)
    {
        return {
            {},
    ...
    vk::PresentModeKHR ChoosePresentMode(std::vector<vk::PresentModeKHR> const& availableModes)
    {
        ...
        if (auto const found = std::ranges::find_first_of(preferredModeOrder, availableModes); found != preferredModeOrder.end())
        {
            return *found;
        }
...
void BasicTriangleApplication::createSurface()
{
    if (auto const result = static_cast<vk::Result>(glfwCreateWindowSurface(
        m_instance, m_window, nullptr, reinterpret_cast<VkSurfaceKHR*>(&m_surface)));
...
VKAPI_ATTR VkBool32 VKAPI_CALL BasicTriangleApplication::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT vkMessageSeverity,
